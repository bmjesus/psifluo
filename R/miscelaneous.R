...
pw_level<- function(protocol){
  ...
  if (protocol=='RLC'){
    ...
    if (red==1){
      power.level <- meta.data[1,"F_Voltage"]
      #print(power.level)
      power <- cali[which(cali[,"Power.Level"]==power.level),"Red.Flash"]*meta.data[1,"MeasuringFlash"]*constants$photons.in.umol*constants$meter.sq.in.angstrom.sq
    }
    if (blue==1){
      power.level <- meta.data[1,"F2_Voltage"]
      #print(power.level)
      power <- cali[which(cali[,"Power.Level"]==power.level),"Blue.Flash"]*meta.data[1,"MeasuringFlash"]*constants$photons.in.umol*constants$meter.sq.in.angstrom.sq
    }
  }
  #print (power)
