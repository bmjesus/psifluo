...
fit_sti_simple<-function(x,y,fo.opt,fm.opt,sigma.opt,rho.opt,sigma_max,sti.fit){
  ...
  lsfit.light <- tryCatch({
    minpack.lm::nlsLM(y ~ psiworx.model.sti(incident=x, sigma=sigma.opt, fo=fo.opt, fm=fm.opt, rho=rho.opt),  start=fg, algorithm="port", upper=ub, lower=lb, trace=T,
    ...
    print(paste("Error: could not fit with model ",".",sep=""));
    return(NULL)
  });
